...
export function evaluateMinimumSalary(
  text: string,
  minimumPkr = 80_000,
) {
  ...
  const matches = [
    ...haystack.matchAll(
      /(?:rs\.?|pkr|pakistani rupees?|salary|pay|compensation)?\s*\$?\s*(\d+(?:\.\d+)?)\s*(lacs?|lakhs?|k|usd|dollars?)?/gi,
    ),
  ];
    ...
    .map((match) => {
      ...
      const normalizedUnit =
        unit.startsWith("lac") || unit.startsWith("lakh")
          ...
          : unit === "k"
            ? "k"
            : unit.includes("usd") || unit.includes("dollar") || match[0].includes("$")
              ? "usd"
              : value >= 1000
                ? "pkr"
                : value <= 300
